...
from models import db, User, Customer, Loan, EMISchedule, LoanAuditLog, Collection
from datetime import datetime
from utils.interest_utils import calculate_flat_emi, calculate_reducing_emi, generate_dates
...
def create_loan():
    ...
    existing_loan = Loan.query.filter_by(customer_id=customer_id, status='active').first()
    if existing_loan and not data.get('override_active_loan'):
        return jsonify({"msg": "Customer already has an active loan", "code": "DUPLICATE_LOAN"}), 400
...
def approve_loan(id):
    ...
    if loan.interest_type == 'reducing':
        schedule_data = calculate_reducing_emi(loan.principal_amount, loan.interest_rate, loan.tenure, loan.tenure_unit)
    else:
        schedule_data = calculate_flat_emi(loan.principal_amount, loan.interest_rate, loan.tenure, loan.tenure_unit)
