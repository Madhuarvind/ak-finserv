...
from models import db, User, UserRole, Customer, Loan, Line, Collection, DailySettlement, CustomerVersion, CustomerNote, CustomerDocument, SystemSetting
...
def ai_analyst():
    ...
    if any(greet in query for greet in ['hi', 'hello', 'hey', 'vanakkam']):
        response["text"] = "Hello! I am your AI Financial Analyst. You can ask me about collections, agent performance, or risk. For example: 'What is today's total cash?'"
        return jsonify(response), 200

    # 1. Total Collections Query (All time or general)
    if 'total' in query and ('collection' in query or 'collect' in query or 'all' in query):
        total_sum = db.session.query(db.func.sum(Collection.amount)).filter_by(status='approved').scalar() or 0
        total_count = Collection.query.filter_by(status='approved').count()
        response["text"] = f"Our lifetime approved collections reached ₹{total_sum:,.2f} across {total_count} entries. This reflects a healthy recovery rate across all lines."
        response["data"] = {"value": total_sum, "metric": "LifeTime Collections", "count": total_count}
    ...
    # 2. Today's Cash or just "Total cash"
        ...
        today_cash = db.session.query(db.func.sum(Collection.amount)).filter(
            db.func.date(Collection.created_at) == today,
            Collection.status == 'approved',
            Collection.payment_mode == 'cash'
        ).scalar() or 0
        
        today_upi = db.session.query(db.func.sum(Collection.amount)).filter(
            db.func.date(Collection.created_at) == today,
            Collection.status == 'approved',
            Collection.payment_mode == 'upi'
        ).scalar() or 0

        today_count = Collection.query.filter(db.func.date(Collection.created_at) == today, Collection.status == 'approved').count()
        
        top_agent_today = db.session.query(
            User.name, db.func.sum(Collection.amount)
        ).join(Collection, User.id == Collection.agent_id)\
         .filter(db.func.date(Collection.created_at) == today, Collection.status == 'approved')\
         .group_by(User.id).order_by(db.func.sum(Collection.amount).desc()).first()

        if 'upi' in query:
             response["text"] = f"Today's total approved UPI collection is ₹{today_upi:,.2f}."
             ...
             response["data"] = {"cash": today_cash, "upi": today_upi, "total": today_cash + today_upi}
        
        response["type"] = "metric"

    # 3. Best/Top Agent (All time)
    elif 'top' in query or 'best' in query or 'agent' in query or 'performance' in query:
        ...
        if top_agent:
            response["text"] = f"Our all-time top performing agent is {top_agent[0]}, who has successfully recovered ₹{top_agent[1]:,.2f}. This agent consistently maintains high geofencing accuracy."
    ...
    # 4. Defaults / High Risk
        ...
        high_risk_count = Loan.query.filter(Loan.pending_amount > (Loan.principal_amount * 0.8)).count()
        response["text"] = f"I've identified {high_risk_count} loans with a potential high risk of default (over 80% balance remaining)."
