...
def get_todays_status():
    ...
    for agent in agents:
        ...
        total_cash = db.session.query(func.sum(Collection.amount))\
        ...
        settlement = DailySettlement.query.filter_by(agent_id=agent.id, date=today).first()
        ...
        if settlement:
            agent_data.update({
                ...
                "verified_at": settlement.verified_at.isoformat() if settlement.verified_at else None
            })
...
def verify_settlement():
    ...
    system_cash = db.session.query(func.sum(Collection.amount))\
    ...
    return jsonify({"msg": "Settlement verified successfully", "difference": difference}), 200
